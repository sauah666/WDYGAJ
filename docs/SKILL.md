...
## Инвариант 6 — Диегетический Интерфейс и Нарративная Целостность (Lessons Learned)

### Определение
Пользовательский интерфейс (UI) автономного агента не является просто "панелью управления"; он является **диегетическим устройством** (существующим внутри мира системы), которое:
*   Олицетворяет состояние системы (например, "сон", "пробуждение", "работа").
*   Создает ментальную модель "Машины", а не "Веб-страницы".
*   Обеспечивает эмоциональную связь через персонификацию (Agent Persona).

### Обязательные действия
Архитектор обязан:
*   Выбрать единую визуальную метафору (напр. "Стимпанк-Планшет", "Терминал Хакера").
*   Запретить использование стандартных UI-компонентов (Bootstrap, Material), если они нарушают метафору.
*   Синхронизировать анимации переходов с изменениями стейт-машины (state machine).

### Правило
**State First, Visuals Second, but tightly coupled.**
Визуальное состояние (анимация, видео) никогда не должно противоречить логическому состоянию (`AgentStatus`).
*   *Ошибка*: Орб "спит", хотя статус `ANALYZING`.
*   *Ошибка*: Интерфейс доступен, пока идет загрузка контекста.

### Проверка
Если удаление CSS/Видео ломает понимание того, что делает агент — архитектура UI слишком зависима от декораций.
Если наличие CSS/Видео скрывает ошибки логики — декорации вредны.
Баланс: Логика работает "вслепую" (headless), но UI делает её понятной человеку.

---

## Инвариант 7 — Протокол Завершения Задачи (Task Completion Protocol)

### Определение
Каждая сессия взаимодействия с человеком должна завершаться структурированным отчетом. Это обеспечивает прозрачность изменений и упрощает проверку.

### Обязательные действия
После выполнения технических задач (написание кода, рефакторинг, фиксы) архитектор обязан предоставить отчет в следующем формате:

1.  **Анализ Проблемы**: Краткое описание того, что было сломано или требовалось сделать.
2.  **Внесенные Изменения**: Список затронутых файлов и суть изменений (патч).
3.  **Верификация**: Почему это решение работает и не нарушает инварианты.

Этот отчет выводится в конце ответа пользователю (natural language).
