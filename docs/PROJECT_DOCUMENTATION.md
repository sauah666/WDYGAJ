...
Итого: конфигурация завершена; дальше начинается исполнение (клики, сбор вакансий, отклики).
...
- Проверка, что фильтры реально применены
- Если не совпало: 3 попытки корректировки → error и останов
...
4) Текущая точка
Последний завершённый этап: ЭТАП 5.4 — BUILD_APPLY_SEARCH_PLAN
Текущий этап: PHASE A1.1 — APPLY_PLAN_STEP_EXECUTION (ONE STEP)
Следующий этап: PHASE A1.2 — APPLY_SEARCH_PLAN (ALL STEPS)

---

## Progress Update — PHASE A1.1

### WHAT WAS ADDED
*   **Entities:** `AppliedFiltersSnapshotV1`, `AppliedStepResult` для трекинга прогресса выполнения.
*   **BrowserPort:** Метод `applyControlAction` — абстракция для выполнения UI-действия (set, toggle, click) над абстрактным полем.
*   **UseCase:** `executeSearchPlanStep` — логика выборки следующего невыполненного шага из плана и его исполнения.
*   **UI:** Кнопка "EXECUTE NEXT STEP" и визуализация статуса каждого шага в плане.

### WHY
Этот этап необходим для безопасного перехода от "планирования" к "действиям". Пошаговое исполнение позволяет отладить взаимодействие с элементами управления до запуска автоматического цикла, исключая риск бесконечных кликов или потери контекста.

### CONSTRAINTS
*   Исполняется ровно один шаг за вызов.
*   LLM не задействован.
*   Никакого анализа результатов поиска (вакансий) на этом этапе.

### NOTES / RISKS
*   Текущая реализация полагается на `domHint` в `SearchUISpec`. Если DOM изменился между анализом (Stage 5.3) и исполнением (Phase A), действие может упасть.
*   В будущем потребуется механизм Drift Detection.
